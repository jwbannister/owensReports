...
if (TRUE){
    excluded_sites = paste(c(1920, 1921, seq(1938, 1940), seq(1942, 1947), 1960, 1963, 1964))
...
for (i in report_index){
    ...
    if (area=='twb2'){
        ...
        comment_file <- 
            gsub(" ", "_", paste0(area, month(start_date), year(start_date), ".txt"))
    }
    ...
    if (area=='twb2' & i!="East" & paired_flag){
        ...
        if (nrow(tbl_pair)==0){
            cat(" \n\n###No PM10 increases across area recorded. \n")
        } else{
        pandoc.table(as.data.frame(tbl_pair), split.table=Inf)
        }
        # Comment relevant to April 2019 report only
#        cat(" \n The upwind-downwind concentration difference observed on 04/02/2019 is derived from 
#            a 2-hour wind event across the Tillage areas. This concentration difference is much less than 50 
#            micrograms/m<sup>3</sup> when distributed to a 24-hour average as specified in the Stipulated Judgement. This event is noted for informational purposes but does not qualify as an exceedance of the TwB2 requirements per the Stipulated Judgement.")
        cat("<p style=\"page-break-after:always;\"></p> \n")
    }
...
#        pandoc.table(as.data.frame(tbl_surface), split.table=Inf)
        cat("</h6> \n")
        cat(" \n\n## Comments \n")
        cat(" \n A TwB2 surface survey was not conducted in October 2019, in order to ensure the 
            sand catch collections would be completed on time.  The next survey is scheduled for the 
            week of November 11 to 15, 2019.\n")
...
#        cat(" \n RS and RH results are reported as rounded integer numbers. RS/RH ratio reported above is calculated from the actual (decimal) values.\n")
        if (i=='South'){
#            cat(" \n The portion of DCA T3NE encompassing sites 1616 and 1617 was undergoing maintenance 
#                in September. No surface measurements were taken at these sites in September 2019.\n")
        }
