...
for (i in index){
    ...
    if (area=='sfwcrft'){
        ...
        if (area %in% c("channel", "t1a1")){
            ...
            cat(paste0("\n\nNote that the ", area_index[area], " not subject to a 
        ...
        if (area=='twb2'){
            ...
            tbl_mass <- csc_mass %>% filter(id3==i) %>% select(csc, sand.mass)
    ...
    if (area!='sfwcrft'){
        ...
        if (nrow(tbl_flux_list[[split_size]]) + nrow(tbl_mass) + 
            comment_length < 35){
            cat(" \n\n## Monthly Sand Mass \n")
            pandoc.table(tbl_mass)
        } else if (nrow(tbl_flux_list[[split_size]]) + nrow(tbl_mass) < 35){
            cat(" \n\n## Monthly Sand Mass \n")
            pandoc.table(tbl_mass)
            cat("<p style=\"page-break-after:always;\"></p> \n")
            report_header(start_date, end_date, report_date, area)
