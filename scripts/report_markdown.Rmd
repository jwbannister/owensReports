...
index <- unique(csc_locs$id3)[!is.na(unique(csc_locs$id3))]
```
...
for (i in index){
    ...
    if (area=='twb2'& exists('surface_df')){
        ...
        cat("</h6> \n")
        cat(" \n\n## Comments \n")
        cat(paste0(" \n Surface data not collected from sites ",
                   paste(missing_surface$site, collapse=", "), ".\n"))
        comment_file_surface <- paste0(area, month(start_date), year(start_date), i, 
                               "_surf.txt")
