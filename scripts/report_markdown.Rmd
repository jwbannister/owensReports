...
for (i in index){
    ...
    gdrive_comments <- system(paste0(getwd(), "/gdrive list ", 
                      "-q \"'0B8qHESXOhs-DXzJfRXdmUXoyWGs' in parents\""), intern=T)  
    cmnt_fl <- tempfile()
    write.table(gdrive_comments, file=cmnt_fl, quote=F, row.names=F, col.names=F)
    comment_list <- read.table(cmnt_fl, header=F, skip=1)
    ...
    if (area=='sfwcrft'){
        if (!is.na(swir_fl)){
            ...
            comment_file <- paste0(area, month(start_date), year(start_date), i, 
                                   "_wet.txt")
            if (comment_file %in% comment_list[ , 2]){
                index <- which(comment_list[ , 2]==comment_file)
                system(paste0(getwd(), "/gdrive download --path ", tempdir(), " ", 
    ...
    if (area=='twb2' & i!="East"){
        ...
        if (start_date=='2017-06-01' & i=='North'){
            cat(" \n\n## Comments \n")
            cat(" \n T29-4 sand flux sites were removed in May 2017 for area 
                maintenance.\n")
        }
