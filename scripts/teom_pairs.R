...
teom_met <- pull_teom_wind(start_date, end_date)
# convert wind speed from mph to m/s
teom_met$ws <- teom_met$ws * .44704 
...
deploys <- as.character(unique(df0$deployment.id))
teom_locs <- pull_locations(deploys)

pm10_df <- pull_pm10(start_date, end_date, deploys)

teom_data <- inner_join(df0, select(teom_locs, deployment.id, deployment),
                  by="deployment.id") %>%
