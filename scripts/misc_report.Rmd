...
if (nrow(filter(flux_df, sand_flux<0))>0){
...
img_files <- vector(mode="character", length=length(names(flux_grobs)))
names(img_files) <- names(flux_grobs)
...
# TEMPORARY CHANGE FOR FEBRURAY BRINE REPORT ONLY
csc_mass[csc_mass$csc %in% c('1702', '1703', '1704', '1706', '1708', 
                             '1709', '1710', '1713'), ]$sand.mass <- "Site Removed"
...
for (i in names(flux_grobs)){
    ...
    cat(" \n\n## Daily Sand Flux \n")
    tbl_flux <- full_daily %>% filter(dca==i, sand.flux>1) %>% 
    ...
    if (area=="channel"){
    cat("\n\nNote that the Channel Areas are not subject to a specific sand flux 
        threshold. However, this report includes dates when sand flux was 
        observed in excess of 1.0 g/cm<sup>2</sup>/day; the most 
        conservative sand flux threshold applied to the approved Best Approved 
        Control Measures.\n")
    }
    if (area=="t1a1"){
    cat("\n\nNote that T1A-1 is not subject to a specific sand flux 
        threshold. However, this report includes dates when sand flux was 
        observed in excess of 1.0 g/cm<sup>2</sup>/day; the most 
        conservative sand flux threshold applied to the approved Best Approved 
        Control Measures.\n")
    ...
    cat(" \n\n## Monthly Sand Mass \n")
    tbl_mass <- csc_mass %>% filter(dca==i) %>% select(csc, sand.mass)
    if (area=="dwm"){
